...
const customTooltips = function (context) {
  ...
  if (!this._chart.canvas.closest('.tickzick-chart-container').contains(tooltipEl)) {
    ...
    document.querySelectorAll('.tickzick-chart-container').forEach((el) => {
      if (el.contains(document.querySelector('.chartjs-tooltip'))) {
        document.querySelector('.chartjs-tooltip').remove();
      }
    });

    this._chart.canvas.closest('.tickzick-chart-container').appendChild(tooltipEl);
  }
