...
function TaskList({ taskStatus, header, description, taskLimit }) {
  ...
  return (
    <div className="relative">
      <div className="bg-white dark:bg-white10 rounded-10 min-h-[220px] tickzick-tassklist-wrap">
        {header === '' ? (
          ''
        ) : (
          <div className="tickzick-tasklist-head">
            <h2 className="text-[16px] font-medium px-[25px] py-[15px] border-b border-normal dark:border-white10 mb-[25px] tickzick-tasklist-head__title">
              {header}
            </h2>
          </div>
        )}

        <div className="tickzick-tasklist-body px-[25px] pb-[25px]">
          {/* Favourite Task */}
          {taskStatus === 'favourite' ? (
            task.filter((item) => item.favourite).length > 0 ? (
              <ul className="tickzick-tasklist">
                {task
                  ...
                  .map((item, i) => {
                    return (
                      <li
                        className="flex items-center justify-between py-[7.5px] gap-[10px] flex-wrap tickzick-tasklist-item "
                        key={i}
                      >
                        <div className="tickzick-tasklist-item__content">
                          <div className="tickzick-tasklist-item__title text-[16px] text-dark dark:text-white87">
                            <Checkbox
                          ...
                          {description ? (
                            <div className="ltr:pl-[30px] rtl:pr-[30px] tickzick-tasklist-item__text">
              ...
              <ul className="tickzick-tasklist">
                {task
                  ...
                  .map((item, i) => {
                    return (
                      <li
                        className="flex items-center justify-between py-[7.5px] gap-[15px] flex-wrap tickzick-tasklist-item "
                        key={i}
                      >
                        <div className="tickzick-tasklist-item__content">
                          <div className="text-[16px] text-dark dark:text-white87 tickzick-tasklist-item__title">
                            <Checkbox
                              className="text-[15px] font-normal text-theme-gray dark:text-white60 relative top-[-2px] [&>.ant-checkbox-checked>.ant-checkbox-inner]:border-success [&>.ant-checkbox-checked>.ant-checkbox-inner]:bg-success"
                              checked={!!item.completed}
                              onChange={() => dispatch(onCompleteUpdate(task, item.id))}
      ...
      <Modal
        title="Edit Task"
        className="tickzick-addTask-modal"
        ...
        forceRender
      >
        <div className="tickzick-addTask-modal-inner">
          {task
            .sort((a, b) => b.id - a.id)
              ...
              return (
                <div key={i}>
                  <Form form={form} name="add-task" onFinish={(eData) => onEditHandle(eData, value.id)}>
                    ...
                    <div className="tickzick-modal-actions">
