...
function MenuItems({ toggleCollapsed }) {
  ...
  // Local state for layout modes
  const [layoutMode, setLayoutMode] = useState('lightMode');
  const [topMenu, setTopMenu] = useState(false);
  const [directionMode, setDirectionMode] = useState('ltr');
  const [authRole, setAuthRole] = useState('admin');

  // Get auth role from cookie on component mount
  useEffect(() => {
    const getAuthRoleFromCookie = () => {
      try {
        const cookieValue = Cookies.get('auth_role');
  ...
  const changeNavbar = (topMode) => {
    ...
    if (topMode) {
      html.classList.add('tickzick-topmenu');
    } else {
      html.classList.remove('tickzick-topmenu');
    }
    setTopMenu(topMode);
  };
  ...
  const adminItems = [
    ...
    getItem(t('Event Management'), 'event-management', !topMenu && <UilCalendarAlt />, [
      getItem(
    ...
    getItem(t('Event Bookings'), 'event-bookings', !topMenu && <UilTicket />, [
      ...
      getItem(
        <NavLink onClick={toggleCollapsed} to={`${path}/bookings?status=completed`}>
      ...
      getItem(
        <NavLink onClick={toggleCollapsed} to={`${path}/bookings?status=pending`}>
          {t('pending bookings')}
        </NavLink>,
        'pending-bookings',
        null,
      ...
      getItem(
        <NavLink onClick={toggleCollapsed} to={`${path}/bookings?status=rejected`}>
          {t('rejected bookings')}
        </NavLink>,
        'rejected-bookings',
        null,
      ...
      getItem(
        <NavLink onClick={toggleCollapsed} to={`${path}/bookings/report`}>
          {t('report')}
        </NavLink>,
        'report',
        null,
    ...
    getItem(t('Support Tickets'), 'support-tickets', !topMenu && <UilCommentAlt />, [
      getItem(
    ...
    getItem(
      <NavLink onClick={toggleCollapsed} to={`${path}/profile/edit`}>
        {t('edit profile')}
      </NavLink>,
      'edit profile',
      !topMenu && <UilUserCircle />,
    ),
    getItem(
      <NavLink onClick={toggleCollapsed} to={`${path}/profile/change-password`}>
        {t('change password')}
      </NavLink>,
      'change password',
      !topMenu && <UilLock />,
    ...
    //     null,
    //   ),
    //   getItem(<NavLink to={`${path}/components/drag`}>Drag & Drop</NavLink>, 'drag', null),
    //   getItem(
  ...
  const organizerItems = [
    ...
    getItem(t('Event Management'), 'event-management', !topMenu && <UilCalendarAlt />, [
      getItem(
        <NavLink onClick={handleEventCreate}>
          {t('add event')}
        </NavLink>,
        'add-event',
        null,
    ...
    getItem(t('Support Tickets'), 'support-tickets', !topMenu && <UilCommentAlt />, [
      ...
      getItem(
        <NavLink onClick={toggleCollapsed} to={`${path}/support-tickets/create`}>
          {t('add ticket')}
        </NavLink>,
        'add-ticket',
        null,
  ...
  const items = authRole === "admin" ? adminItems : organizerItems;
